...
def migratedb_command():
    """Detect the need for and apply any database migrations."""
    db = get_db()
...
def is_enabled(username: str) -> bool:
    """Returns True if the user is enabled, False if they are not."""
    db = get_db()
    if username is None:
        return False
    return db.get_user_by_name(username)[3] == 1
...
def admin_authenticated(decoratee):
    """Decorator to check whether the user is authenticated and an administrator"""
...
def login():
    if not app.config["DISABLE_AUTH"]:
        ...
        if "sso" in request.args:
            ...
            session_index = (
                session["samlSessionIndex"] if "samlSessionIndex" in session else None
            )
            ...
            if len(errors) == 0:
                ...
                session["log_rows"] = 3
                if not is_user(session["username"]) or not is_enabled(session["username"]):
                    session["last_error"] = (
                        "Unfortunately, you're not an "
                ...
                session["last_error"] = (
                    "There was an error while handling the"
                    " SAML response: " + str(auth.get_last_error_reason())
                )
                return redirect(url_for("error"))
        ...
        print(
            "WARNING: AUTHENTICATION IS DISABLED. IF THIS MESSAGE APPEARS "
            "IN YOUR PRODUCTION LOGS, SOMETHING IS WRONG."
        )
        session["username"] = (
            "tst1234" if not app.config["AUTH_NAME_TOGGLE"] else "wel2138"
        )
...
def get_csv_logs(how_many_rows: int) -> str:
    ...
    for count in counts:
        ...
        counts_as_string += count["submit_time"] + "," + count["entered_time"] + ","
        room_counts = {}
        for row in room_rows:
...
def show_help():
    ...
    return render_template("help.html", buttons=buttons)
...
def error():
    ...
    return render_template("error.html", message=error_msg, buttons=buttons)
